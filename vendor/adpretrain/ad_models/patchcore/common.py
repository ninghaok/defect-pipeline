...
try:
    import faiss  # optional: the pipeline uses TorchNN by default
...
class TorchNN(object):
    """Exact chunked nearest-neighbour search without FAISS OpenMP calls."""

    def __init__(self, device="cpu", query_chunk_size=128):
        self.device = torch.device(device)
        self.query_chunk_size = int(query_chunk_size)
        self.index_features = None

    def fit(self, features: np.ndarray) -> None:
        self.index_features = torch.as_tensor(
    ...
    def run(self, n_nearest_neighbours, query_features, index_features=None):
        references = (
            ...
            if index_features is None
                ...
                index_features, dtype=torch.float32, device=self.device
            ).contiguous()
        )
        if references is None:
            raise RuntimeError("TorchNN.fit must be called before run")
        distance_chunks, index_chunks = [], []
        with torch.inference_mode():
            for start in range(0, len(query_features), self.query_chunk_size):
                query = torch.as_tensor(
                    query_features[start : start + self.query_chunk_size],
                    dtype=torch.float32,
                    device=self.device,
                )
                squared_distances = torch.cdist(query, references, p=2).square()
                distances, indices = torch.topk(
                    squared_distances,
                    k=n_nearest_neighbours,
                    dim=1,
                    largest=False,
                    sorted=True,
                )
                distance_chunks.append(distances.cpu().numpy())
                index_chunks.append(indices.cpu().numpy())
        return np.concatenate(distance_chunks), np.concatenate(index_chunks)

    def save(self, filename: str) -> None:
        np.save(filename + ".npy", self.index_features.cpu().numpy())

    def load(self, filename: str) -> None:
        self.fit(np.load(filename + ".npy"))
...
class RescaleSegmentor:
    ...
    def convert_to_segmentation(self, patch_scores):
        ...
        with torch.no_grad():
            ...
            sigma = float(self.smoothing)
            radius = max(1, int(round(4.0 * sigma)))
            coordinates = torch.arange(
                -radius, radius + 1, device=_scores.device, dtype=_scores.dtype
            )
            ...
            _scores = F.pad(
                _scores, (radius, radius, 0, 0), mode="reflect"
            )
            _scores = F.conv2d(_scores, horizontal)
            _scores = F.pad(
                _scores, (0, 0, radius, radius), mode="reflect"
            )
            _scores = F.conv2d(_scores, vertical)
...
class NearestNeighbourScorer(object):
    def __init__(self, n_nearest_neighbours: int, nn_method=None) -> None:
        ...
        if nn_method is None:
            nn_method = FaissNN(False, 4) if faiss is not None else TorchNN()
